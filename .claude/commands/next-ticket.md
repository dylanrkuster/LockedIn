...
Automatically find and start the next open ticket in the backlog with gated approvals.
...
1. **Find the next open ticket**:
   - Scan `tickets/*.yml` in numeric order (001, 002, 003...)
   - Find the first ticket with `status: open`
   - If no open tickets, inform user and stop

2. **Display ticket summary**:
   - Show ticket ID, title, type, and priority
   - Ask user to confirm before starting

3. **On confirmation, create feature branch**:
   ```bash
...
6. **Initialize state** in SCRATCHPAD.md:
   ```markdown
   ## Current Ticket: [ID]
   ### Phase: PROPOSAL (1/6)
...
## Notes
- Tickets are processed in ID order (lowest first)
- Only considers tickets with `status: open`
- Skips tickets with `status: closed`, `status: in_progress`, etc.
